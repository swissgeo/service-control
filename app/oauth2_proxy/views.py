...
def admin_logout(request: HttpRequest) -> HttpResponse:
    ...
    redirect_after_logout = settings.DJANGO_ADMIN_LOGOUT_REDIRECT_URL or base_url(request)
    ...
    # We need to log out (chained with redirects) from oauth2-proxy -> eIAM -> Cognito
    eiam_logout_url = (
        f"{settings.OAUTH2_PROXY_EIAM_LOGOUT_URL}?"
        f"post_logout_redirect_uri={quote_plus(cognito_logout_url)}"
    )

    oauth_proxy_logout_url = (
        f"{base_url(request)}{settings.OAUTH2_PROXY_URL_PREFIX}sign_out?"
        f"rd={quote_plus(eiam_logout_url)}"
    )
