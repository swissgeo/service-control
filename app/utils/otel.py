...
def _get_exporters() -> tuple[
    ...
    return log_exporters, span_exporters
...
log_exporters, span_exporters = _get_exporters()
...
def reinitialize_otel() -> None:
    ...
    After forking, all background threads used by BatchSpanProcessor and BatchLogRecordProcessor
    are dead in the child process. This function creates fresh ones with new background threads.
    """
    global log_provider, trace_provider  # noqa: PLW0603

    new_log_provider, new_trace_provider = _get_providers()
    new_log_exporters, new_span_exporters = _get_exporters()
