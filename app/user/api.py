...
async def machine_users(
    ...
    models = MachineUser.objects.filter(organization__organization_id=organization_id).order_by(
        "name"
    )
    response = [machine_user_to_response(model) async for model in models]
...
async def delete_machine_users(
    request: HttpRequest,  # noqa: ARG001  request is not used but required by ninja
