...
async def add_extra_audience(aud: str) -> None:
    ...
    ssm_client = Client()
    param_value = await ssm_client.get_parameter(param_name)
    param_value = aud if not param_value else param_value + "," + aud
    await ssm_client.put_parameter(param_name, param_value)


async def remove_extra_audience(aud: str) -> None:
    ...
    await ssm_client.put_parameter(param_name, new_value)
